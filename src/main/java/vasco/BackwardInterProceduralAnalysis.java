...
public abstract class BackwardInterProceduralAnalysis<M,N,A> extends InterProceduralAnalysis<M,N,A> {
			...
			if (node != null) {
				...
				if (programRepresentation().isCall(node)) {
					...
					for (M targetMethod : programRepresentation().resolveTargets(currentContext.getMethod(), node)) {
						...
						VascoContext<M,N,A> targetContext = getContext(targetMethod, exitValue);
	 ...
	 */
	protected VascoContext<M,N,A> initContext(M method, A exitValue) {
		// Construct the context
		VascoContext<M,N,A> context = new VascoContext<M,N,A>(method, programRepresentation().getControlFlowGraph(method), true);
	 ...
	 */
	public abstract A normalFlowFunction(VascoContext<M,N,A> context, N node, A outValue);
	 ...
	 */
	public abstract A callEntryFlowFunction(VascoContext<M,N,A> context, M targetMethod, N node, A entryValue);
	 ...
	 */
	public abstract A callExitFlowFunction(VascoContext<M,N,A> context, M targetMethod, N node, A outValue);
	 ...
	 */
	public abstract A callLocalFlowFunction(VascoContext<M,N,A> context, N node, A outValue);
