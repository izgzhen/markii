...
public abstract class OldForwardInterProceduralAnalysis<M,N,A> extends InterProceduralAnalysis<M,N,A> {
	...
	public OldForwardInterProceduralAnalysis() {
		...
		super(false);
		analysisStack = new Stack<VascoContext<M,N,A>>();
	}
	
	protected Stack<VascoContext<M,N,A>> analysisStack;
	...
	public void doAnalysis() {
		...
		for (M entryPoint : programRepresentation().getEntryPoints()) {
			VascoContext<M,N,A> context = new VascoContext<M,N,A>(entryPoint, programRepresentation().getControlFlowGraph(entryPoint), false);
		...
		while (!analysisStack.isEmpty()) {
			...
			VascoContext<M,N,A> context = analysisStack.peek();
			...
			if (!context.getWorkList().isEmpty()) {
				...
				if (unit != null) {
					...
					if (freeResultsOnTheFly) {
						Set<VascoContext<M,N,A>> reachableContexts = contextTransitions.reachableSet(context, true);
	 ...
	 */
	protected void initContext(VascoContext<M,N,A> context, A entryValue) {
		// Get the method
		...
		if (!contexts.containsKey(method)) {
			contexts.put(method, new LinkedList<VascoContext<M,N,A>>());
		}
	 ...
	 * is instead called by {@link #flowFunction(VascoContext, Object, Object) flowFunction} when a method
	 ...
	 * an invoke expression when implementing {@link #flowFunction(VascoContext, Object, Object) flowFunction},
	 * and suitably invoke {@link #processCall(VascoContext, Object, Object, Object) processCall} with
	 * the input data flow value which may be different from the IN/OUT value of the node due to
	 * handling of arguments, etc. Similarly, the result of {@link #processCall(VascoContext, Object, Object, Object) processCall}
	 * may be modified by the client to handle return values, etc. before returning from  {@link #flowFunction(VascoContext, Object, Object) flowFunction}.
	 * Ideally,  {@link #flowFunction(VascoContext, Object, Object) flowFunction} should return
	 * <tt>null</tt> if and only if {@link #processCall(VascoContext, Object, Object, Object) processCall}
	 * returns <tt>null</tt>.
	 * 
	 ...
	 */
	protected A processCall(VascoContext<M,N,A> callerContext, N callNode, M method, A entryValue) {
		...
		VascoContext<M,N,A> calleeContext = getContext(method, entryValue);
		// If not, then set 'calleeContext' to a new context with the given entry flow.
		if (calleeContext == null) {
			calleeContext = new VascoContext<M,N,A>(method, programRepresentation().getControlFlowGraph(method), false);
	...
	protected abstract A flowFunction(VascoContext<M,N,A> context, N unit, A in);
