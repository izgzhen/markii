...
public abstract class InterProceduralAnalysis<M,N,A> {
	...
	protected final NavigableSet<VascoContext<M,N,A>> worklist;

	/** A mapping from methods to a list of contexts for quick lookups. */
	protected final Map<M,List<VascoContext<M,N,A>>> contexts;
	 ...
	 */
	public VascoContext<M,N,A> getContext(M method, A value) {
		...
		if (reverse) {
			...
			for (VascoContext<M,N,A> context : contexts.get(method)) {
	 ...
	 */
	public List<VascoContext<M,N,A>> getContexts(M method) {
		if (contexts.containsKey(method)) {
			return Collections.unmodifiableList(contexts.get(method));
		} else {
			return Collections.unmodifiableList(new LinkedList<VascoContext<M,N,A>>());
		}
	}
